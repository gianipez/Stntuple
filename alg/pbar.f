...
c-----------------------------------------------------------------------
      block data
      include "Stntuple/alg/pbar_common.inc"
c
      data ifirst_time/0/
      data p2max/1.3/
      data p2c  /1.1/
...
c-----------------------------------------------------------------------
c reworked pbar_yield: d^2Sigma/dP/dcosth (lab)
c-----------------------------------------------------------------------
      function pbar_d2n(pbeam,p,tet)

      include "Stntuple/alg/pbar_common.inc"

c     d^2Sigma/dP/dcos(Theta) of pbar production with momentum P (GeV/c)
c     and angle TET (rad) in proton-nucleus collision.
c     normalized to the pA inelastic cross section on Tantalum (1.539e6 mubarn)
